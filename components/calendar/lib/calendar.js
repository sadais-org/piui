...
class Calendar {
  ...
  _currentMonthDys(dateData, full) {
    ...
    for (let i = 1; i <= dateData; i++) {
      ...
      // 日期的每一天
      let nowDate =
        full.year + '-' + (full.month < 10 ? full.month : full.month) + '-' + (i < 10 ? '0' + i : i)
      // 是否今天
      const toDay = this.getDate().fullDate
      let isDay = toDay === nowDate
      // 获取打点信息
      let info =
        this.selected &&
  ...
  getlunar(year, month, date) {
    return lunarCalendar.solar2lunar(year, month, date)
  }
  /**
   * 设置打点
   */
  ...
  setMultiple(fullDate) {
    ...
    if (before && after) {
      ...
      /**
       * 2020/2/28 修改 当重新选取范围时 直接赋值为开始时间 优化体验
       */
