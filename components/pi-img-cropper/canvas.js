export default {
  ...
  methods: {
    ...
    draw() {
      ...
      this.$emit('beforeDraw', { ctx, width: this.crop.width, height: this.crop.height })
      ctx.fillRect(0, 0, this.crop.width * this.canvasZoom, this.crop.height * this.canvasZoom)
      // 将画布的坐标原点 移动到裁剪窗口左上角对应的那个像素点的位置
      ctx.translate((this.img.width * 0.5 + this.img.transform.x - this.crop.left) * this.canvasZoom, (this.img.height * 0.5 + this.img.transform.y - this.crop.top) * this.canvasZoom)
      // 旋转图片
      ctx.rotate(this.img.transform.rotate)
      // 将坐标原点移动回左上角
      ctx.translate(-this.img.width * 0.5 * this.img.transform.scale * this.canvasZoom, -this.img.height * 0.5 * this.img.transform.scale * this.canvasZoom)

      ctx.drawImage(this.img.path, 0, 0, this.img.width * this.img.transform.scale * this.canvasZoom, this.img.height * this.canvasZoom * this.img.transform.scale)
      // 绘制完裁剪图片之后
      // @arg ctx 结构: {ctx: canvas实例, width, height}
      ctx.restore()
      this.$emit('afterDraw', { ctx, width: this.crop.width * this.canvasZoom, height: this.crop.height * this.canvasZoom })
      ctx.draw(false, () => {
